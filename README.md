...
如意翻译是一款正在开发的 uTools 文本翻译插件。用户明确提交文本后，插件使用用户自己的 AI 服务密钥生成译文；术语、结构和准确性风险检查会按仓库工单逐步加入。

当前版本已经可以创建 DeepSeek Flash 默认配置、加密保存用户填写的 API Key，并在用户确认完整发送地址和数据范围后完成一次标准全文翻译。已确认文档：
